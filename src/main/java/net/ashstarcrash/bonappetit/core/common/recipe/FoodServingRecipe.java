...
public class FoodServingRecipe extends CustomRecipe {
        ...
        for (int index = 0; index < input.size(); ++index) {
            ...
            if (!selectedStack.isEmpty()) {
                if (cookingPotStack.isEmpty()) {
                    ItemStack mealStack = CookingPotEntity.getMealFromItem(selectedStack);
                    if (!mealStack.isEmpty()) {
                        cookingPotStack = selectedStack;
                        containerStack = CookingPotEntity.getContainerFromItem(selectedStack);
                        continue;
                    }
                }
    ...
    public ItemStack assemble(CraftingInput input, HolderLookup.Provider access) {
        for (int i = 0; i < input.size(); ++i) {
            ...
            if (!selectedStack.isEmpty() && selectedStack.is(BAItems.COOKING_POT.get())) {
                ItemStack resultStack = CookingPotEntity.getMealFromItem(selectedStack).copy();
    ...
    public NonNullList<ItemStack> getRemainingItems(CraftingInput input) {
        ...
        for (int i = 0; i < remainders.size(); ++i) {
            ...
            } else if (selectedStack.is(BAItems.COOKING_POT.get())) {
                CookingPotEntity.takeServingFromItem(selectedStack);
    ...
    public RecipeSerializer<?> getSerializer() {
        return BARecipeSerializers.FOOD_SERVING.get();
    }
}