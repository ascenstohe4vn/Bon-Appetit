        ...
        if (event.getSource().getEntity() instanceof LivingEntity attacker) {
            ...
            if (cherryEffect != null && !event.getSource().is(BADamageTypes.TWIN_HIT)) {
                ...
                if (charge >= 0.9f && cooldown <= 0) {
                    ECHO_QUEUE.add(new CherryEcho(attacker, victim, (float)((damage * BAConfig.TWIN_STRIKE_INITIAL_DAMAGE_MULTIPLIER.get()) + (cherryAmplifier * BAConfig.TWIN_STRIKE_ADDITIVE_DAMAGE_MULTIPLIER.get())), 5));
